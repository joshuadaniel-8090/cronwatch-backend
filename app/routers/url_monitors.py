...
from app.schemas.url_monitor import URLMonitorCreate, URLMonitorUpdate, URLMonitorResponse, URLMonitorLogResponse
...
def create_url_monitor(body: URLMonitorCreate, user_id: str = Depends(get_current_user_id)):
    ...
    if profile["plan"] == "free" and body.check_interval_seconds < 600:
        raise HTTPException(
            status_code=403,
            detail="Free plan minimum check interval is 10 minutes (600s). Upgrade to Pro for 1 minute checks."
        )
...
def update_url_monitor(id: uuid.UUID, body: URLMonitorUpdate, user_id: str = Depends(get_current_user_id)):
    ...
    if "check_interval_seconds" in update_data:
        profile_res = supabase.table("profiles").select("plan").eq("id", user_id).single().execute()
        if profile_res.data["plan"] == "free" and update_data["check_interval_seconds"] < 600:
            raise HTTPException(status_code=403, detail="Free plan minimum interval is 10 minutes.")
